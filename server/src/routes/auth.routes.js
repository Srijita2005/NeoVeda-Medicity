 ...
 * 
 * POST /api/auth/login          â€” Employee Code + Password login
 * POST /api/auth/refresh        â€” Refresh access token
 * GET  /api/auth/me             â€” Resolve current session user profile
 * POST /api/auth/change-password â€” Change password (authenticated)
 * POST /api/auth/logout         â€” Clear session cookies
 */
...
/**
 * GET /api/auth/me
 * Returns hydrated current user profile from JWT session identity.
 */
router.get('/me', authenticate, async (req, res) => {
    try {
        const user = await authService.getSessionUser(req.user);
        return successResponse(res, { user }, 'Session user resolved.');
    } catch (err) {
        return errorResponse(res, err.message || 'Failed to resolve session user.', err.status || 500);
    }
});
